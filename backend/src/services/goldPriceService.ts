...
import { GoldPriceResponse } from '../types/product';
...
export class GoldPriceService {
  ...
  public async getGoldPrice(): Promise<number> {
    ...
    try {
      // Metal API'den altın fiyatını çek
      ...
      if (response.data && response.data.length > 0) {
        ...
        // API'den veri gelmezse varsayılan değer kullan
        const defaultPrice = 2000; // USD/oz varsayılan değer
  ...
  public convertOzToGram(pricePerOz: number): number {
    return pricePerOz / 31.1035; // 1 oz = 31.1035 gram
  }
} 